-- The till shouldn't need a manual "Abrir caja" click before the day's first
-- sale — open one automatically (fondo inicial $0) the moment a sale needs it
-- and none is open yet. Manual open/close still works exactly as before for
-- when the owner wants to declare a real starting float.
