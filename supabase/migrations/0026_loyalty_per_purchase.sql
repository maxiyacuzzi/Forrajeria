-- Loyalty target is 10 PURCHASES of a product, not 10 bags' worth of
-- quantity — buying 500g counts the same toward the goal as buying a whole
-- bag. (Previously fraccionable products accumulated a kg-equivalent
-- total, so a big purchase counted proportionally more than a small one;
-- that's no longer the rule.) The 50% discount still lands on whatever line
-- item triggered it, whole bag or loose grams, unchanged.
...
-- Retroactively recomputes every customer_product_loyalty row by replaying
-- that customer+product's real purchase history (one sale_items row = one
-- purchase) with the corrected per-purchase rule, since existing rows were
-- accumulated under the old kg-equivalent rule and no longer mean the same
-- thing under this one.
do $$
declare
  v_pair record;
  v_purchase record;
...
begin
  for v_pair in
    select distinct s.customer_id, si.product_id
    from public.sale_items si
    join public.sales s on s.id = si.sale_id
  loop
    v_progress := 0;
    v_last_purchase := null;

    for v_purchase in
      select s.created_at
      from public.sale_items si
      join public.sales s on s.id = si.sale_id
      where s.customer_id = v_pair.customer_id and si.product_id = v_pair.product_id
      order by s.created_at, si.id
    loop
      if v_last_purchase is not null and v_purchase.created_at - v_last_purchase > interval '40 days' then
        v_progress := 0;
      end if;
      ...
      v_last_purchase := v_purchase.created_at;
    end loop;

    update public.customer_product_loyalty
    set progress_qty = v_progress, last_purchase_at = v_last_purchase, updated_at = now()
    where customer_id = v_pair.customer_id and product_id = v_pair.product_id;
  end loop;
end $$;
