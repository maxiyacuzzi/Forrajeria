-- 10% surcharge on sales paid with "tarjeta" (not posnet_mp — Mercado Pago's
-- fees work differently and aren't passed on to the customer the same way).
-- Applied on top of the post-loyalty-discount amount, since that's what's
-- actually run through the card.

alter table public.sales
  add column surcharge_amount numeric(14, 2) not null default 0;

create or replace function public.create_sale(
  p_customer_id uuid, p_items jsonb, p_note text default null, p_payment_method public.payment_method default 'efectivo'
)
...
begin
  ...
  if v_cash_register_id is null then
    begin
      insert into public.cash_registers (org_id, opening_amount, opened_by)
      values (v_org_id, 0, auth.uid())
  ...
  if p_payment_method = 'tarjeta' then
    v_surcharge := round((v_subtotal - v_discount) * 0.10, 2);
  end if;
  ...
  set subtotal_amount = v_subtotal,
      ...
      total_amount = v_subtotal - v_discount + v_surcharge,
