-- Loyalty is meant for alimento (food, sold in bulk — bolsas/kg/fardos), not
-- accesorios (collares, juguetes, sold as one whole "simple" unit). Existing
-- "simple" products stop earning it retroactively (that's the closest
-- existing signal to "is this a bulk food item"); fraccionable/no_fraccionable
-- products keep earning it. Still editable per product for exceptions.

alter table public.products
  add column earns_loyalty boolean not null default true;

update public.products set earns_loyalty = false where unit_type = 'simple';

-- Drop stale progress for products that no longer earn loyalty.
delete from public.customer_product_loyalty cpl
using public.products p
where p.id = cpl.product_id and p.earns_loyalty = false;

-- create_sale skips loyalty progress/reward entirely for items whose product
-- doesn't earn it — no customer_product_loyalty row, no discount eligibility.
-- Body is otherwise unchanged from migration 0026.
